...
class CharadesCaptionDataset(DatasetAbstract):
    def __init__(self, config: CfgNode) -> None:
        super().__init__()
        self.config = config
        self.dataset_root_path = config.DATA.ROOT_PATH
        self.train_csv = os.path.join(self.dataset_root_path, config.DATA.TRAIN_CSV)
        self.val_csv = os.path.join(self.dataset_root_path, config.DATA.VAL_CSV)
    ...
    def get_train_dataset(self) -> Dataset:
        ...
        # )
        clip_sampler = pytorchvideo.data.make_clip_sampler("random", self.clip_duration)
        train_dataset = CustomCharades(data_path=self.train_csv, 
    ...
    def get_val_dataset(self) -> Dataset:
        ...
        # )   
        clip_sampler = pytorchvideo.data.make_clip_sampler("uniform", self.clip_duration)
        val_dataset = CustomCharades(data_path=self.val_csv, 
                            clip_sampler=clip_sampler,
                            transform=self.val_transforms,
                            tokenizer=self.tokenizer)
        return val_dataset
